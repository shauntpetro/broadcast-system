...
function broadcast(type, data, options = {}) {
  ...
  // Some messages should be sent immediately (e.g., connection status, pin updates, slideshow sync, widget updates)
  const immediateTypes = ['status', 'error', 'init', 'slideshow_init', 'slideshow_update', 'sports_ticker_init', 'sports_ticker_update', 'state_delta', 'pin', 'queue_update', 'nametag_update', 'social_update', 'lower_third_update', 'agenda_update', 'topic_card_update'];
...
/**
 * Broadcast nametag update
 * @param {object} nametags - All nametag slot data
 */
function broadcastNametagUpdate(nametags) {
  broadcast('nametag_update', nametags, { immediate: true });
}

/**
 * Broadcast social accounts update
 * @param {object} data - Social accounts data { socialAccounts, showSocials, socialRotationSpeed }
 */
function broadcastSocialUpdate(data) {
  broadcast('social_update', data, { immediate: true });
}

/**
 * Broadcast lower third update
 * @param {object} data - Lower third data { visible, headline, description }
 */
function broadcastLowerThirdUpdate(data) {
  broadcast('lower_third_update', data, { immediate: true });
}

/**
 * Broadcast agenda update
 * @param {object} data - Agenda data { agendaItems, agendaTitle, showAgenda }
 */
function broadcastAgendaUpdate(data) {
  broadcast('agenda_update', data, { immediate: true });
}

/**
 * Broadcast topic card update
 * @param {object} data - Topic card data { visible, title, subtitle }
 */
function broadcastTopicCardUpdate(data) {
  broadcast('topic_card_update', data, { immediate: true });
}
...
module.exports = {
  ...
  broadcastNametagUpdate,
  broadcastSocialUpdate,
  broadcastLowerThirdUpdate,
  broadcastAgendaUpdate,
  broadcastTopicCardUpdate,
  addClient,
  removeClient,
  getClientCount
};
